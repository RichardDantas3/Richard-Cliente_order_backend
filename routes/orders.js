...
router.get('/',async(req,res) =>{
    try{
        const orders = await pool.query(
            'SELECT o.*,c.names as client_name,c.email as client_email FROM orders o JOIN clients c ON o.client_id = c.client_id ORDER BY o.created_at DESC'
        );
        res.json(orders);
    } catch (error) {
...
router.post('/', async(req, res) =>{
    ...
    if (!client_id || !order_date || !total_amount){
        return res.status(400).json({error:"Cliente ID, order date, and total amount are required"});
    }
    try{
